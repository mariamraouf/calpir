      ...
      <div className="container text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {steps.map((step, index) => (
            <div
              key={index}
              className="relative flex flex-col items-center text-center p-8 bg-gray-900 dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-700 dark:border-gray-700 hover:shadow-2xl transition-shadow duration-300 transform hover:scale-110 animate-fade-in-up" /* Darker card, border */
            ...
            >
              ...
              <div className="mt-8">
                ...
                <h3 className="text-xl font-semibold mb-3 text-white dark:text-white animate-fade-in-up" style={{ animationDelay: `${0.6 + index * 0.2}s` }}>{step.title}</h3> {/* White text */}
                <p className="text-gray-400 dark:text-gray-400 animate-fade-in-up" style={{ animationDelay: `${0.7 + index * 0.2}s` }}>{step.description}</p> {/* Lighter gray text */}
              </div>
            </div>
          ))}
        </div>
