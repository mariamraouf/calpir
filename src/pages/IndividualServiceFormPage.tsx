  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-grow">
        <PageHeader
          title="Just Need a Specific Boost? Grab Individual Services Here!"
          description="Need a targeted solution for your business without committing to a full package? No problem! Pick the exact services you need, and our experts will dive in to give you tailored recommendations and get things set up with precision and speed. We're here to help you tackle those specific goals."
          buttons={[
                          ...
                          {services.map((service) => (
                            <div key={service.id} className="flex items-start space-x-2 p-3 rounded-2xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 cursor-pointer hover:shadow-md hover:border-primary transition-all duration-300">
                              ...
                              <Label htmlFor={`service-${service.id}`} className="flex flex-col flex-grow cursor-pointer">
                                ...
                                <Link to={`/pricing#${service.id}`} className="text-primary hover:underline text-xs mt-1" target="_blank" rel="noopener noreferrer">
                                  (Learn More)
                                </Link>
                              </Label>
                            </div>
