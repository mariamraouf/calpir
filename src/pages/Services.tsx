    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-grow">
        ...
        <div className="md:sticky md:top-[80px] z-40 w-full bg-background dark:bg-gray-950 py-4 shadow-sm">
          <div className="container flex flex-wrap items-center justify-center rounded-full bg-white dark:bg-gray-800 p-1 shadow-xl border border-gray-200 dark:border-gray-700 gap-2 sm:gap-1">
            {Object.keys(groupedServices).map((category, index) => (
              <a key={index} href={`#${category.toLowerCase().replace(/\s/g, '-')}-section`} className="w-full sm:w-auto">
                <Button
        ...
        {/* Section for Individual Services, grouped by category */}
        {Object.entries(groupedServices).map(([category, subcategories], catIndex) => (
          <section key={category} id={`${category.toLowerCase().replace(/\s/g, '-')}-section`} className="container py-16 md:py-24 scroll-mt-24">
            <h2 className="text-3xl md:text-4xl font-bold text-center mb-10 text-gray-900 dark:text-white animate-fade-in-up" style={{ animationDelay: `${0.3 + catIndex * 0.1}s` }}>
              {category}
            </h2>
            <p className="text-lg text-gray-600 dark:text-gray-300 max-w-4xl mx-auto leading-relaxed text-center mb-12 animate-fade-in-up delay-200">
              Explore our specialized services within {category.toLowerCase()} to find the perfect solution for your business needs.
            </p>
            {Object.entries(subcategories).map(([subcategory, services], subcatIndex) => (
              <div key={subcategory} className="mb-12 last:mb-0">
                <h3 className="text-2xl font-bold text-center mb-8 text-gray-800 dark:text-gray-200 animate-fade-in-up" style={{ animationDelay: `${0.4 + catIndex * 0.1 + subcatIndex * 0.05}s` }}>
                  {subcategory}
                </h3>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                  {services.map((service, serviceIndex) => (
                    <div
                      key={service.id}
                      id={service.id} // Anchor for direct linking
                      className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 hover:shadow-2xl transition-shadow duration-300 transform hover:scale-105 animate-slide-in-left"
                      style={{ animationDelay: `${0.5 + catIndex * 0.1 + subcatIndex * 0.05 + serviceIndex * 0.03}s` }}
                    >
                      ...
                      <ul className="list-none space-y-3 text-left mb-8">
                        {service.details.map((detail, detailIndex) => (
                          <li key={detailIndex} className="flex items-start text-gray-700 dark:text-gray-300 animate-fade-in-up" style={{ animationDelay: `${0.6 + catIndex * 0.1 + subcatIndex * 0.05 + serviceIndex * 0.03 + detailIndex * 0.02}s` }}>
